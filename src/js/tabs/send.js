                                           ...
                                           keychain)
  {
      ...
      if (send.federation) {
        ...
        $federation.check_email(recipient)
          .then(function (result) {
            ...
            var nowRecipient = send.recipient_address;
            if (recipient !== nowRecipient) return;
            ...
            // Check if this request is still current, exit if not
            var nowRecipient = send.recipient_actual || send.recipient_address;
            if (recipient !== nowRecipient) return;
      ...
      account.entry(function (e, data) {
        $scope.$apply(function () {
          ...
          if (e) {
            ...
            var reserveBase = $scope.account.reserve_base;
            send.xrp_deficiency = reserveBase.subtract(data.account_data.Balance);
    ...
    $scope.update_amount = function () {
      ...
      if (send.quote_url) {
        ...
        var reserveBase = $scope.account.reserve_base;
        if (total.compareTo(reserveBase) < 0) {
          send.fund_status = 'insufficient-xrp';
          send.xrp_deficiency = reserveBase.subtract(send.recipient_info.Balance);
        }
        ...
        $.ajax({
          ...
          success: function (data) {
            $scope.$apply(function () {
              ...
              var nowAmount = send.amount_feedback;
              if (!nowAmount.equals(send.amount_feedback)) return;
    ...
    $scope.update_paths = function () {
      ...
      pf.on('update', function (upd) {
        $scope.$apply(function () {
          ...
          var nowAmount = send.amount_actual || send.amount_feedback;
          if (!nowAmount.equals(amount)) return;
