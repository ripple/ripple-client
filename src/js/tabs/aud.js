...
AudTab.prototype.angular = function(module) {
  module.controller('AudCtrl', ['$scope', 'rpId', 'rpAppManager', 'rpTracker', '$routeParams', 'rpKeychain', 'rpNetwork', '$timeout',
    function($scope, id, appManager, rpTracker, $routeParams, keychain, $network, $timeout)
    {
     ...
     $scope.save_account = function() {
        ...
        var amount = ripple.Amount.from_human(
            Options.gateway_max_limit + ' ' + 'AUD', {
                reference_date: new Date(+new Date() + 5 * 60000)
            }
        );

        amount.set_issuer('rsP3mgGb2tcYUrxiLFiHJiQXhsziegtwBc');
      ...
      $scope.$watch('lines', function() {
        if ($scope.lines.rsP3mgGb2tcYUrxiLFiHJiQXhsziegtwBcAUD) {
          $scope.audConnected = true;
        } else {
          $scope.audConnected = false;
        }
      }, true);
