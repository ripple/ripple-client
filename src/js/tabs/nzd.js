...
NzdTab.prototype.angular = function(module) {
  module.controller('NzdCtrl', ['$scope', 'rpId', 'rpAppManager', 'rpTracker', '$routeParams', 'rpKeychain', 'rpNetwork', '$timeout',
    function($scope, id, appManager, rpTracker, $routeParams, keychain, $network, $timeout)
    {
      ...
      $scope.$watch('lines', function() {
        if ($scope.lines.rsP3mgGb2tcYUrxiLFiHJiQXhsziegtwBcNZD) {
          $scope.nzdConnected = true;
        } else {
          $scope.nzdConnected = false;
        }
      }, true);
