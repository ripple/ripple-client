                                        ...
                                        $location, $timeout, rpHistory)
{
  ...
  $scope.loadNotification = function(status) {
   ...
   */
  function processTxn(tx, meta, isHistoric)
  {
    ...
    if (processedTxn && processedTxn.error) {
      ...
      rpTracker.trackError('JsonRewriter Error', err, {
        ...
        Source: isHistoric ? 'app controller historic' : 'app controller event'
      });
      ...
      // Update Ripple lines
      if (processedTxn.effects && !isHistoric) {
  ...
  function updateLines(effects)
  {
    ...
    $.each(effects, function () {
      if (_.contains([
          ...
          'trust_change_no_ripple'
        ], this.type))
      {
        var _this = this,
          line = {},
          index = _this.counterparty + _this.currency;

        line.currency = _this.currency;
        line.account = _this.counterparty;
        line.flags = _this.flags;
        line.no_ripple = !!_this.noRipple; // Force Boolean

        if (_this.balance) {
          line.balance = _this.balance;
          updateRippleBalance(
            _this.currency,
            _this.counterparty,
            _this.balance
        ...
        if (_this.limit) {
          line.limit = _this.limit;
        }

        if (_this.limit_peer) {
          line.limit_peer = _this.limit_peer;
        }
  ...
  function updateRippleBalance(currency, newAccount, newBalance)
  {
    ...
    if (newAccount) {
      balance.components[newAccount] = newBalance;
    }
    ...
    appManager.getApp(newAccount, function(err, app) {
      if (err) {
        console.warn(err);
        return;
      }
      ...
      balance.components[newAccount].gateway = gateway;
      ...
      app.findProfile('account').getUser($scope.address, function(err, user) {
        ...
        gateway.inboundBridge.getInstructions($scope.address, function(err, instructions) {
          ...
          //   gateway.inboundBridge.limit = gateway.inboundBridge.currencies[0].limit - balance.components[newAccount].to_human();
          // }
  ...
  if (!store.disabled && !store.get('ripple_pairsAll')) {
    store.set('ripple_pairsAll', require('../data/pairs'));
  }

  var pairsAll     = store.get('ripple_pairsAll'),
      pairsDefault = require('../data/pairs');
  $scope.pairsAll  = genericUtils.uniqueObjArray(pairsAll, pairsDefault, 'name');
  ...
  function compareLastUsed(a, b) {
    var timeA = a.last_used || a.order || 0,
        timeB = b.last_used || b.order || 0;
    if (timeA < timeB) return 1;
    if (timeA > timeB) return -1;
    return 0;
  }
  $scope.pairsAll.sort(compareLastUsed);
  ...
  $scope.$watch('pairsAll', function() {
    if (!store.disabled) {
      store.set('ripple_pairsAll', $scope.pairsAll);
    }
  }, true);

  $scope.pairs = $scope.pairsAll.slice(1);
  $timeout(function() {
  ...
  $scope.generate_src_currencies = function () {
    ...
    srcCurrencies.push({ currency: 'XRP' });
    for (var currencyName in balances) {
      if (!balances.hasOwnProperty(currencyName)) continue;

      var currency = balances[currencyName];
      var currencyHex = currency.total.currency().to_hex();
      var result = [];
      for (var issuerName in currency.components)
      {
        if (!currency.components.hasOwnProperty(issuerName)) continue;
        var component = currency.components[issuerName];
        if (component.is_positive())
          result.push({ currency: currencyHex, issuer: issuerName});
      }

      if (result.length > 1 || isIssuer[currencyHex] || result.length === 0)
        result.unshift({ currency: currencyHex });

      srcCurrencies = srcCurrencies.concat(result);
    }
